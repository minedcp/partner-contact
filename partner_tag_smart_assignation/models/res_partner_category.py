...
class ResPartnerCategory(models.Model):
    ...
    ], 'JOIN operator', default='or', required=True)
    ...
    def check_sql_query(self):
        for me in self.filtered('tag_filter_sql_query'):
            ...
            for row in rows:
                if len(row) > 1 or not isinstance(row[0], int) or not \
                        self.env['res.partner'].browse(row[0]).exists():
                    raise ValidationError(
    ...
    def get_partners_from_ir_filter(self):
        ...
        if matching_records:
            if model == 'res.partner':
                ...
                partners = matching_records.mapped(
                    self.tag_filter_partner_field)
            return partners
        else:
            return self.env['res.partner']
